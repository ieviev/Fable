...
module AL =
    type ALWriter(com: Compiler, cliArgs: CliArgs, pathResolver, targetPath: string) =
        let sourcePath = com.CurrentFile
        let fileExt = cliArgs.CompilerOptions.FileExtension
        let stream = new IO.StreamWriter(targetPath)
        interface Printer.Writer with
    ...
    let compileFile (com: Compiler) (cliArgs: CliArgs) pathResolver isSilent (outPath: string) = async {
        let php =
            FSharp2Fable.Compiler.transformFile com
            |> FableTransforms.transformFile com
            |> Fable2AL.Compiler.transformFile com

        if not (isSilent || ALPrinter.isEmpty php) then
            use writer = new ALWriter(com, cliArgs, pathResolver, outPath)
            do! ALPrinter.run writer php
    }
...
let compileFile (com: Compiler) (cliArgs: CliArgs) pathResolver isSilent (outPath: string) =
    ...
    | AL -> AL.compileFile com cliArgs pathResolver isSilent outPath
