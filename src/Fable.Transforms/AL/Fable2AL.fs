...
let convertModuleDecl (com:IALCompiler) (moduleDecl:Fable.ModuleDecl) =
    let entRef = moduleDecl.Entity
    let ent = com.GetEntity(entRef)
    match ent.Attributes |> Seq.length with
    | 0 -> moduleDecl.Members |> List.collect (convertDecl com) 
    | _ ->
        match getModuleKind ent with
        | Default -> [] 
        | Codeunit ->
            let members = moduleDecl.Members
            let converted = [ for m in members do convertMemberDecl com m ]
            let t = 1
            []


let rec convertDecl (com: IALCompiler)  decl : ALDecl list =
    match decl with
    | Fable.Declaration.ModuleDeclaration moduleDecl ->
        convertModuleDecl com moduleDecl
