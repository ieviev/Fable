module Fable.Transforms.Fable2AL
...
let rec convertType (com: IALCompiler)  (t: Fable.Type) =
    match t with
...
/// be done in Php)
let caseName (com: IALCompiler) (entity: Fable.Entity) (case: Fable.UnionCase) =
...
//let convertSingleCaseUnion (com: IALCompiler) (decl: Fable.ClassDecl) (info: Fable.Entity) =
...
//let  convertMultiCaseUnion (com: IALCompiler) (decl: Fable.ClassDecl) (info: Fable.Entity) =
//    let baseType =
//            { Namespace = Some com.PhpNamespace
//              Name = fixName decl.Name
//              Fields = []
...
//let convertUnion (com: IALCompiler) (decl: Fable.ClassDecl) (info: Fable.Entity) =
...
//let convertRecord (com: IALCompiler) (decl: Fable.ClassDecl) (info: Fable.Entity) =
//    let t =
//        { Namespace = Some com.PhpNamespace
//          Name = fixName decl.Name
...
//let rec convertTypeRef  (com: IALCompiler) (t: Fable.Type) =
//    match t with
...
//    | Fable.Unit ->
//        ExType phpVoid
//and getPhpTypeForEntity (com: IALCompiler) (entity: Fable.Entity) =
...
//let libCall (com: IALCompiler) file ns memberName args =
//
//
...
//let convertTest (com: IALCompiler)  test phpExpr =
...
//let rec convertExpr (com: IALCompiler) (expr: Fable.Expr) =
//    match expr with
...
//and convertFunction (com: IALCompiler)  body (args: Fable.Ident list) =
//    com.NewScope()
//    let args =
//        [ for arg in args do
...
//and convertValue (com: IALCompiler)  (value: Fable.ValueKind) range =
//    match value with
...
//and convertMatching (com: IALCompiler) input guard thenExpr elseExpr expr returnStrategy =
...
//and convertExprToStatement (com: IALCompiler) expr returnStrategy =
//    match expr with
...
//let convertMemberDecl (com: IALCompiler) (decl: Fable.MemberDecl) =
//    
//    let name =
...
let convertDecl (com: IALCompiler)  decl =
    match decl with
    | n ->
...
module Compiler =
    ...
    let transformFile com (file: Fable.File) =
        let phpComp = ALCompiler(com) :> IALCompiler
        let decls =
            [
                for i,decl in List.indexed file.Declarations do
                    let decls =
                        try
                            // todo: proper convertDecl
//                            convertDecl phpComp decl
                            match decl with
                            | Fable.Declaration.ClassDeclaration classDecl ->
                                [ALDecl.ALObject classDecl.Name]
                            | Fable.Declaration.MemberDeclaration memberDecl ->
                                [ALDecl.ALMember memberDecl.Name]
        ...
        {
            Filename = phpComp.CurrentFile + ".al"
            Namespace = Some ""
            Require = []
            Uses = []
            Decls = decls
